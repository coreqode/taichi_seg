...
def __render_rays_test(model, rays_o, rays_d, hits_t, **kwargs):
    ...
    seg = torch.zeros(N_rays, 1, device=device)
    ...
    while samples < kwargs.get('max_samples', MAX_SAMPLES):
        ...
        segs = torch.zeros(len(xyzs), 1, device=device)
        sigmas[valid_mask], _rgbs, _segs= model(xyzs[valid_mask], dirs[valid_mask],
                                          **kwargs)
        rgbs[valid_mask] = _rgbs.float()
        segs[valid_mask] = _segs.float()
        sigmas = rearrange(sigmas, '(n1 n2) -> n1 n2', n2=N_samples)
        rgbs = rearrange(rgbs, '(n1 n2) c -> n1 n2 c', n2=N_samples)
        segs = rearrange(segs, '(n1 n2) c -> n1 n2 c', n2=N_samples)

        composite_test_fw(sigmas, rgbs, segs, deltas, ts, hits_t[:,
                                                           0], alive_indices,
                          kwargs.get('T_threshold', 1e-4), N_eff_samples,
                          opacity, depth, rgb, seg)
    ...
    for k, v in kwargs.items():  # supply additional inputs, repeated per ray
        if isinstance(v, torch.Tensor):
            kwargs[k] = torch.repeat_interleave(v[rays_a[:, 0]], rays_a[:, 2],
                                                0)
    sigmas, rgbs, seg_out = model(xyzs, dirs, **kwargs)

    (results['vr_samples'], results['opacity'],
    results['depth'], results['rgb'], results['seg'], results['ws']) = \
        model.render_func(sigmas, rgbs, seg_out, results['deltas'], results['ts'],
