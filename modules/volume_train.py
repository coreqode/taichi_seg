...
@ti.kernel
def composite_train_fw(sigmas: ti.template(), rgbs: ti.template(), segs:ti.template(),
                       ...
                       rgb: ti.template(), seg: ti.template(), ws: ti.template()):
...
class VolumeRenderer(torch.nn.Module):
    ...
    def __init__(self, batch_size=8192, data_type=data_type):
        ...
        # samples level
        ## 1024 is number of samples per ray
        ...
        self.rgbs_fields = ti.field(dtype=data_type,
                                    ...
                                    needs_grad=True)
        self.segs_fields = ti.field(dtype=data_type,
                                    shape=(batch_size * 1024, 1),  ## Need to change with n_labels
                                    needs_grad=True)
        ...
        self.rgb_fields = ti.field(dtype=data_type,
                                   shape=(batch_size, 3),
                                   needs_grad=True)
        self.seg_fields = ti.field(dtype=data_type,
                                   shape=(batch_size, 3),
                                   needs_grad=True)
        ...
        self.register_buffer(
            'seg',
            torch.zeros(batch_size, 1, dtype=torch_type),
            persistent=False
        )
        self.register_buffer(
            'opacity',
        ...
        self.register_buffer(
            'seg_grad', torch.zeros(batch_size * 1024, 1, dtype=torch_type),
            persistent=False
        )
        ...
        class _module_function(torch.autograd.Function):
            ...
            def forward(ctx, sigmas, rgbs, segs, deltas, ts, rays_a, T_threshold):
                ...
                torch2ti(self.segs_fields, segs.contiguous())
                ...
                composite_train_fw(self.sigmas_fields, self.rgbs_fields, self.segs_fields,
                                   ...
                                   self.rgb_fields, self.seg_fields, self.ws_fields)
                ...
                ti2torch(self.seg_fields, self.seg)
            ...
            def backward(ctx, dL_dtotal_samples, dL_dopacity, dL_ddepth,
                ...
                seg_grad = self.seg_grad[:samples_size].contiguous()
                ...
                torch2ti_grad(self.seg_fields, dL_dseg.contiguous())
                torch2ti_grad(self.ws_fields, dL_dws.contiguous())
                composite_train_fw.grad(self.sigmas_fields, self.rgbs_fields, self.segs_fields,
                                        self.deltas_fields, self.ts_fields,
                ...
                ti2torch_grad(self.segs_fields, seg_grad)

                return sigma_grad, rgb_grad, seg_grad, None, None, None, None
    ...
    def forward(self, sigmas, rgbs, segs, deltas, ts, rays_a, T_threshold):
        return self._module_function.apply(sigmas, rgbs, segs, deltas, ts, rays_a,
                                           T_threshold)
